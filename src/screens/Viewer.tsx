...
export default function Viewer() {
    ...
    const dirPath = route.params
        ? route.params.dirPathList[route.params.dirIdx]
        : RNFS.DocumentDirectoryPath + '/manwa';
    ...
    useEffect(() => {
        ...
        RNFS.readDir(dirPath).then((result) => {
            ...
            result
                ...
                .forEach((file, i) =>
                    newImgs.push({ id: i + 1, fileName: file.name })
                );
        ...
        AsyncStorage.getItem(dirPath).then((result) => {
            if (!(result && result !== '1'))
                return AsyncStorage.setItem(dirPath, '1');
    ...
    return (
        <>
            ...
            {!isScrollMode && imgs.length > 0 && (
                <TouchViewer imgs={imgs} dirPath={dirPath} />
            )}
            <ViewerMenu
                dirPathList={route.params?.dirPathList}
                dirIdx={route.params?.dirIdx}
            />
        </>
    );
}
